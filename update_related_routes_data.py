...
def income_metrics(hist, close):
    ...
    # 12 calendar-month distribution consistency.
    buckets=[]
...
def main():
    ...
    for symbol,meta in ROUTES.items():
        try:
            ...
            avg_vol=float(vols.mean()) if not vols.empty else None
            ...
            if meta['type']=='leveraged':
                ...
                if n>=10:
                    ...
                    q['targetDailyMultiple']=multiple
    ...
    data['relatedRoutesDataStatus']='Activity / tracking / income research metrics; public data may be delayed; not exchange real-time; route approval remains separate.'
    data['relatedRoutesFailedSymbols']=failed
    OUT.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print('related route updated',updated,'failed',len(failed))
