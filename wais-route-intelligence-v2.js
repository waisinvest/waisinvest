...
(function(){
  ...
  function ensureShell(){
    ...
    if(main&&!document.getElementById('route-intelligence')){
      ...
      s.innerHTML=`<div class="section-banner"><div><span class="panel-kicker">WAIS EXECUTION VEHICLE INTELLIGENCE</span><h2>Route Intelligence｜投資路線</h2><p>同一 underlying 比較 Stock、⚡ Leveraged、Income。每條 route 獨立 READY；有產品 ≠ 已批准。</p></div></div><article class="panel route-intro-panel"><div class="panel-head"><div><span class="panel-kicker">WAIS ROUTE RULE · REGISTRY ${esc(d.routeRegistry?.version||'2.0')}</span><h3>Verified Route first · Best only after evidence passes</h3></div><div class="route-audit-chip">INDEPENDENT READY</div></div><div class="weekly-risk-note">Stock READY ≠ Leveraged READY ≠ Income READY。已驗證產品即使流動性資料未齊，都應顯示產品名稱並標 DATA GAP / VALIDATING；只有 activity/liquidity/tracking 或 income/NAV/ROC/total-return 證據足夠先可以稱 Best。</div></article><div id="routeIntelligenceGrid" class="route-intelligence-grid"></div>`;
  ...
  function metrics(t,type){
    ...
    if(type==='income'){
      rows.push(metric('T12M Dist. Yield',pct(q.trailing12mDistributionYield)));
      rows.push(metric('30D Income',pct(q.current30dIncomeRate)));
  ...
  function candidate(list,type){
    ...
    if(type==='leveraged') ranked.sort((a,b)=>Number(b.q.avgDollarVolume20d)-Number(a.q.avgDollarVolume20d));
    ...
    const incomeOK=type!=='income'||(finite(x.q.sustainableIncomeYield)&&finite(x.q.incomeConsistency));
    const status=trackingOK&&incomeOK?'VALIDATING · EVIDENCE PRESENT':'VALIDATING · DATA GAP';
    const reason=type==='leveraged'?'Current activity leader among verified products; tracking/spread evidence must remain acceptable before any Best/READY label.':'Current data leader among verified income products; NAV/ROC/total-return validation remains mandatory before any Best/READY label.';
  ...
  function addLinks(){document.querySelectorAll('#topPicksGrid .stock-card,#watchlistCards .watch-card').forEach(card=>{const t=clean(card.querySelector('h3,h4')?.textContent);if(!t||!routes()[t]||card.querySelector('.wais-full-route-btn'))return;const b=document.createElement('button');b.type='button';b.className='route-link-btn wais-full-route-btn';b.textContent='View Full Route →';b.addEventListener('click',()=>openRoute(t));card.appendChild(b);});}
