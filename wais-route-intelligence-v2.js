// WAIS ROUTE INTELLIGENCE v2.4
// Verified product identity is separated from route approval and from data completeness.
// Only underlyings with at least one independently verified leveraged-long, bearish/inverse, or income product are rendered here.
(function(){
  ...
  function ensureShell(){
    ...
    if(main&&!document.getElementById('route-intelligence')){
      ...
      s.innerHTML=`<div class="section-banner"><div><span class="panel-kicker">WAIS EXECUTION VEHICLE INTELLIGENCE</span><h2>Route Intelligence｜投資路線</h2><p>只收錄至少有一條 independently verified Bullish Leveraged、Bearish / Inverse 或 Income product 的 underlying，再獨立比較 Stock、⚡ Bullish、▼ Bearish、Income。純股票名稱留在 Top Picks / Watchlist。</p></div></div><article class="panel route-intro-panel"><div class="panel-head"><div><span class="panel-kicker">WAIS ROUTE RULE · REGISTRY ${esc(d.routeRegistry?.version||'2.4')}</span><h3>Verified Route first · Direction-specific READY</h3></div><div class="route-audit-chip">INDEPENDENT READY</div></div><div class="weekly-risk-note">Stock READY ≠ Bullish Leveraged READY ≠ Bearish READY ≠ Income READY。Bullish / Bearish routes 都必須比較 activity / liquidity / tracking；Income 必須比較 cash yield、frequency、sustainable income、NAV/ROC/total-return。新產品或資料不足只可 VALIDATING / DATA GAP。</div></article><div id="routeIntelligenceGrid" class="route-intelligence-grid"></div>`;
  ...
  function candidate(list,type){
    ...
    let reason='Current evidence leader among verified income products. Products without sufficient distribution history remain alternatives only; NAV/ROC/total-return validation is still mandatory before Best/READY.';
    ...
    if(type==='bearish')reason='Current activity leader among verified bearish/inverse products; a bearish market signal plus tracking/spread evidence is required before Bearish READY. Daily reset makes this tactical, not a passive hedge.';
  ...
  function one(t){
    ...
    const lev=candidate(r.leveraged,'leveraged');
    const bear=candidate(r.bearish,'bearish');
    const inc=candidate(r.income,'income');
  ...
  function render(){const g=document.getElementById('routeIntelligenceGrid');if(g)g.innerHTML=tickers().map(one).join('')||'<article class="panel">No verified leveraged, bearish or income routes yet.</article>';}
