...
SYMBOLS={
"NVDA":("NVDA","USD",False),"TSM":("TSM","USD",False),"AVGO":("AVGO","USD",False),"MRVL":("MRVL","USD",False),"MU":("MU","USD",False),"COHR":("COHR","USD",False),"LITE":("LITE","USD",False),"AXTI":("AXTI","USD",False),"TSEM":("TSEM","USD",False),"POET":("POET","USD",False),"GFS":("GFS","USD",False),"POWL":("POWL","USD",False),"MOD":("MOD","USD",False),"VRT":("VRT","USD",False),"GOOGL":("GOOGL","USD",False),"AAOI":("AAOI","USD",False),"RKLB":("RKLB","USD",False),"AEHR":("AEHR","USD",False),"FORM":("FORM","USD",False),"MXL":("MXL","USD",False),"NVTS":("NVTS","USD",False),"OSS":("OSS","USD",False),"AIRO":("AIRO","USD",False),
# WAIS Route Selector — leveraged route products
"NVDL":("NVDL","USD",False),"NVDU":("NVDU","USD",False),
"GGLL":("GGLL","USD",False),"GOU":("GOU","USD",False),"GOOL":("GOOL","USD",False),
"MUU":("MUU","USD",False),"MULL":("MULL","USD",False),"MIC":("MIC","USD",False),
"AVL":("AVL","USD",False),"TSMX":("TSMX","USD",False),"MRVU":("MRVU","USD",False),
# WAIS Route Selector — income route products
"NVDW":("NVDW","USD",True),"NVYY":("NVYY","USD",True),"NVDY":("NVDY","USD",True),"NVII":("NVII","USD",True),
"GOOW":("GOOW","USD",True),"GOOY":("GOOY","USD",True),"GOOP":("GOOP","USD",True),"GOIB":("GOIB","USD",True),
"MUYY":("MUYY","USD",True),"MUIB":("MUIB","USD",True),"AVGW":("AVGW","USD",True),
...
"QQQI":("QQQI","USD",True),"SPYI":("SPYI","USD",True),"FEPI":("FEPI","USD",True),"AIPI":("AIPI","USD",True),"PLYY":("PLYY","USD",True),"TMYY":("TMYY","USD",True),"XQQI":("XQQI","USD",True)}
...
def income_metrics(h, pos, regular_close, now_utc):
    ...
    if pos.empty or regular_close <= 0:return {}
    today=now_utc.date(); cutoff12=today-timedelta(days=365)
    ...
    month_keys=[];y=today.year;m=today.month
    for _ in range(12):
        month_keys.append((y,m));m-=1
        if m==0:y-=1;m=12
    buckets=[sum(float(v) for idx,v in trail.items() if idx.year==yy and idx.month==mm) for yy,mm in reversed(month_keys)]
    active=[x for x in buckets if x>0];coverage=len(active)/12
    if len(active)>=2:
    ...
    sustainable=max(0.0,min(ttm_yield,median_annual_yield)*stability_factor)
    return {'trailing12mDistribution':round(ttm_cash,6),'trailing12mDistributionYield':round(ttm_yield,4),'current30dIncomeRate':round(rate30,4),'incomeConsistency':round(consistency,1),'sustainableIncomeYield':round(sustainable,4),'sustainableMonthlyEquivalent':round(sustainable/12,4),'distributionCoverageMonths12m':len(active),'incomeMetricMethod':'TTM actual distributions + rolling 30D cash rate + calendar-month consistency + median-month sustainable-yield normalization; research metric, not a forecast or guarantee'}

def main():
    data=existing();prices=data.get('prices',{});ok=0;failed=[];now_utc=datetime.now(timezone.utc)
    for out,(sym,currency,income) in SYMBOLS.items():
        try:
            ...
            regular_close=float(c.iloc[-1]);regular_date=c.index[-1].date().isoformat();display_price=regular_close;display_asof=regular_date;display_session='CLOSE';display_status='Completed daily close'
            try:
                snap=latest_snapshot(ticker)
                if snap:display_price,et,display_session=snap;display_asof=et.isoformat();display_status='Latest available 5-minute snapshot; may be delayed; NOT exchange real-time'
            ...
            if len(c)>=2:
                prev=float(c.iloc[-2]);r['previousClose']=round(prev,4);r['change']=round(display_price-prev,4);r['changePercent']=round((display_price/prev-1)*100,4) if prev else None;first=float(c.iloc[0]);r['priceReturnPeriodPct']=round((regular_close/first-1)*100,4) if first else None
            if income and 'Dividends' in h.columns:
                div=h['Dividends'].fillna(0);pos=div[div>0]
                if not pos.empty:
                    r['lastDistribution']=round(float(pos.iloc[-1]),6);r['lastDistributionDate']=pos.index[-1].date().isoformat();r.update(income_metrics(h,pos,regular_close,now_utc));count=int(len(pos[[x.date()>=now_utc.date()-timedelta(days=365) for x in pos.index]]));r['distributionCount12m']=count;r['observedFrequency']='Weekly' if count>=40 else ('Monthly' if count>=10 else ('Quarterly/Irregular' if count>=3 else 'Sparse/Unknown'));y=r.get('trailing12mDistributionYield');pr=r.get('priceReturnPeriodPct');r['incomeRiskFlag']='VERY HIGH DISTRIBUTION — validate NAV/ROC/total return' if y is not None and y>=25 else ('HIGH DISTRIBUTION — review sustainability' if y is not None and y>=12 else ('5%+ SCREEN MATCH — not an automatic buy' if y is not None and y>=5 else 'Below 5% screen or insufficient data'));r['simpleIncomePlusPriceReturnPct']=round(y+pr,4) if y is not None and pr is not None else None
